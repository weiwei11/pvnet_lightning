...
__global__ void data_copy_kernel(float* new_data, float* old_data,
                            int* start_pos_array, int* element_num_array,
                            int data_num, int data_len)
{
    ...
    for (size_t i = blockIdx.x * blockDim.x + tid; i < data_num; i += blockDim.x * gridDim.x)
    {
        element_num = element_num_array[i];
        end_pos = start_pos_array[i] + element_num;
        for (size_t start_pos = start_pos_array[i]; start_pos < end_pos; start_pos++)
        {
            for (size_t col = 0; col < data_len; col++)
            {
                new_data[start_pos * data_len + col] = old_data[i * data_len + col];
            }
        }
    }
}

at::Tensor data_copy_launcher(
    at::Tensor old_data,     // [n, len]
    at::Tensor start_pos,  // [n,]
    at::Tensor element_num  // [n, ]
)
{
    int data_num = old_data.size(0);
    int data_len = old_data.size(1);
    int total_num = start_pos[data_num-1].item().toInt()+element_num[data_num-1].item().toInt();

    assert(data_num == start_pos.size(0));
    assert(data_num == element_num.size(0));

    int tdim0=1024, tdim1=1, tdim2=1;
    int bdim0=(1023+data_num)/tdim0, bdim1=1, bdim2=1;

    dim3 bdim(bdim0, bdim1, bdim2);
    dim3 tdim(tdim0, tdim1, tdim2);

    auto new_data = at::zeros({total_num, data_len}, old_data.type());
    data_copy_kernel<<<bdim,tdim>>>(
        new_data.data<float>(),
        old_data.data<float>(),
        start_pos.data<int>(),
        element_num.data<int>(),
        data_num,
        data_len
    );
    gpuErrchk(cudaGetLastError())
...
__global__
void generate_radius_hypothesis_kernel(
        float* radius,     // [tn,vn]
        float* coords,     // [tn,2]
        int* idxs,         // [hn,vn,2]
        float* hypo_pts,   // [2*hn,vn,2]
        int tn,
        int vn,
        int hn
)
{
    ...
    float r0=radius[t0*vn+vi];
    float cx0=coords[t0*2];
    float cy0=coords[t0*2+1];

    float r1=radius[t1*vn+vi];
    float cx1=coords[t1*2];
    float cy1=coords[t1*2+1];

    float median_x = (cx0 + cx1) / 2;
    float median_y = (cy0 + cy1) / 2;
    float x_diff = (cx1 - cx0);
    float y_diff = (cy1 - cy0);
    float dist_square = x_diff * x_diff + y_diff * y_diff;
    float dist = sqrt(dist_square);
    float r0_square = r0 * r0;
    float r1_square = r1 * r1;

    // compute intersection points for two circle
    float ix0, iy0, ix1, iy1;
    float item0, item1;
    item0 = (r0_square - r1_square) / (2 * dist_square);
    item1 = 1 / 2 * sqrt(2 * (r0_square + r1_square) / dist_square - (r0_square - r1_square) * (r0_square - r1_square) / (dist_square * dist_square) - 1);
    if (dist_square >= (r0 - r1) * (r0 - r1) && dist_square <= (r0 + r1) * (r0 + r1))
    {
        ix0 = median_x + item0 * x_diff + item1 * y_diff;
        iy0 = median_y + item0 * y_diff + item1 * (-x_diff);
        ix1 = median_x + item0 * x_diff - item1 * y_diff;
        iy1 = median_y + item0 * y_diff - item1 * (-x_diff);
    }
    else
        return;

    hypo_pts[(2*hi)*vn*2+vi*2]=ix0;
    hypo_pts[(2*hi)*vn*2+vi*2+1]=iy0;
    hypo_pts[(2*hi+1)*vn*2+vi*2]=ix1;
    hypo_pts[(2*hi+1)*vn*2+vi*2+1]=iy1;
}

at::Tensor generate_radius_hypothesis_launcher(
        at::Tensor radius,     // [tn,vn]
        at::Tensor coords,     // [tn,2]
        at::Tensor idxs        // [hn,vn,2]
)
{
    int tn=radius.size(0);
    int vn=radius.size(1);
    int hn=idxs.size(0);
    ...
    auto hypo_pts = at::zeros({2*hn,vn,2}, radius.type());
    generate_radius_hypothesis_kernel<<<bdim,tdim>>>(
            radius.data<float>(),
            coords.data<float>(),
            idxs.data<int>(),
            hypo_pts.data<float>(),
    ...
    return hypo_pts;  // [2*hn, vn, 2]
}

__global__
void voting_for_radius_hypothesis_kernel(
        float* radius,     // [tn,vn]
        float* coords,     // [tn,2]
        float* hypo_pts,   // [hn,vn,2]
        unsigned char* inliers,     // [hn,vn,tn]
...
)
{
    ...
    float r = radius[ti*vn + vi];

    float dx=hx-cx;
    float dy=hy-cy;

    float dist = dx * dx + dy * dy;
    if(dist > (r - inlier_thresh) * (r - inlier_thresh) && dist < (r + inlier_thresh) * (r + inlier_thresh))
        inliers[hi*vn*tn+vi*tn+ti]=1;
}


void voting_for_radius_hypothesis_launcher(
        at::Tensor radius,     // [tn,vn]
        at::Tensor coords,     // [tn,2]
        at::Tensor hypo_pts,   // [hn,vn,2]
        at::Tensor inliers,    // [hn,vn,tn]
...
)
{
    ...
    voting_for_radius_hypothesis_kernel<<<bdim,tdim>>>(
            radius.data<float>(),
            coords.data<float>(),
            hypo_pts.data<float>(),
            inliers.data<unsigned char>(),
