...
def crop_or_padding_to_fixed_size_instance_v2(img, mask, hcoords, th, tw, overlap_ratio=0.5):
    h, w, _ = img.shape
    hs, ws = np.nonzero(mask)
    ...
    if hrmin < hrmax:
        ...
        hbeg = 0 if hpad else np.random.randint(hmin, hmax-th+1)
    hend = hbeg + th
    if wrmin < wrmax:
        wbeg = 0 if wpad else np.random.randint(wrmin, wrmax)  # if pad then [0,wend] will larger than [0,w], indexing it is safe
    else:
        wbeg = 0 if wpad else np.random.randint(wmin, wmax-tw+1)
    wend = wbeg + tw
